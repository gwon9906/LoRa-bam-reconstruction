...
폴더는 실험 순서대로 번호가 매겨져 있고, 이름에 접근 방식이 드러나도록 정리되어 있음. `01 → 07` 순서로 읽으면 접근 변경 흐름과 그 결과를 따라갈 수 있음.

```
LoRa-bam-reconstruction/
├── utils/                                 # 공용: LoRa 클래스, BAM 구현, 심볼 추정
├── 01_baseline_stft-magnitude-bam/        # STFT magnitude(dB) + MultiBAM (self-supervised)
├── 02_bam-v3_complex-spectrogram/         # Complex 스펙트로그램 + MultiBAMv3 (Torch 선형) — 출력 collapse
├── 03_bam-v4_tied-autoencoder/            # Complex 스펙트로그램 + MultiBAMv4 (tied AE) — 출력 collapse
├── 04_bam-v5_untied-residual-huber/       # Complex 스펙트로그램 + MultiBAMv5 (untied + outer residual + Huber)
├── 05_bam-v6_residual-fc-blocks/          # Complex 스펙트로그램 + MultiBAMv6 (Residual FC blocks)
├── 06_bam_complex-iq-direct/              # 단층 Complex-Valued BAM (ComplexIQBAM) — IQ 직접 입력
├── 07_noise2noise/                        # Noise2Noise 학습 파이프라인 (신규)
...
### 01_baseline_stft-magnitude-bam/
...
- v3와 동일하게 모든 SNR에서 BAM 정답률 0.1~0.5%로 **출력 collapse 관측됨**. 단일 심볼 디버깅에서도 복원 신호 peak이 클린 대비 0.2% 수준.

### 04_bam-v5_untied-residual-huber/
...
### 07_noise2noise/
- `02_bam-v3_complex-spectrogram/`과 동일한 Complex 스펙트로그램 + `MultiBAMv3` 파이프라인을 self-supervised 방식으로 재활용. 아키텍처는 2-layer로 단축 (7936 → 2048 → 512).
- SF=9, BW=**250 kHz**, OSF=**4**, fs=1 MHz. `dataset_v3_sf9_bw250k/clean_iq/`의 512 clean IQ를 로드, 매 epoch마다 on-the-fly 노이즈로 30배 증강 (`AUGMENT_FACTOR=30`). 학습 SNR randint(-7.5, 0), 30 epochs, batch 64, lr 5e-4.
- `training_FIXED.ipynb`: 위 파이프라인 전체 + 학습 후 가중치를 `weights_bamv3_complex/`에 저장 + 6개 SNR({-30, -25, -20, -15, -10, -5})에 대해 각 1000샘플로 baseline vs BAM 정확도 표/곡선을 산출. 노트북에 저장된 실행 출력이 없어 최종 표는 미확정. 단일 심볼 디버깅(SNR=-20 dB, symbol=42)에서는 baseline/BAM 모두 정답 복원, 다만 복원 peak은 clean 대비 3.6%로 진폭 collapse는 남아 있음.
...
폴더마다 자체 `generate.ipynb`로 클린 IQ를 생성. 노이즈는 폴더에 따라 (a) generate 단계에서 사전 저장(06_bam_complex-iq-direct), (b) 학습 시 on-the-fly 추가(나머지) 중 하나로 구분.

- `dataset_sf9_bw125k_osf8_fs1MHz/{clean_iq, noisy_iq}/sym_XXX*.npy` — 06_bam_complex-iq-direct (사전 noisy 포함)
- `dataset_sf9_bw125k/clean_iq/sym_XXX_iq.npy` — 01_baseline_stft-magnitude-bam
- `dataset_v3_sf9_bw250k/clean_iq/sym_XXX_iq.npy` — 02_bam-v3_complex-spectrogram 및 07_noise2noise (BW=250 kHz, OSF=4)
- `dataset_sf9_bw125k/` 또는 동등 경로 — 03_bam-v4_tied-autoencoder / 04_bam-v5_untied-residual-huber / 05_bam-v6_residual-fc-blocks (on-the-fly augmentation)
